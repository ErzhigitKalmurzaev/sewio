...
import {
  ...
  getFolderList,
  getOperationList,
} from "../../../../../store/technolog/operations";
...
const CreateCombination = ({ modals, setModals }) => {
  ...
  const { folders_list, operaitions_list } = useSelector(
  ...
  const handleAddOperation = (operation) => {
    if (!combination.operations.some((op) => op.id === operation.id)) {
      setCombination({
        ...combination,
        operations: [...combination.operations, operation],
      });
    }
  };
  ...
  const filteredOperations =
    operaitions_list?.results?.filter(
      (op) =>
  ...
  return (
    <Modal
      open={modals?.combination}
      onClose={() => setModals({ ...modals, combination: false })}
    >
      <Modal.Header>
            ...
            {operaitions_list?.results?.length > 0 ? (
              <div className="mt-4 grid grid-cols-2 gap-6">
                <div>
                  <h3 className="text-lg font-medium mb-1">
                    Операции в комбинации
                  </h3>
                  <div className="flex items-end justify-between mb-3">
                    <p>
                      Время:{" "}
                      {combination.operations.reduce(
                        (total, op) => total + op.time,
                        0
                      ) || 0}{" "}
                      c
                    </p>
                    <p>
                      ...
                      {combination.operations
                        ?.reduce((total, op) => total + op.price, 0)
                        ?.toFixed(2) || 0}{" "}
                      cом
                    </p>
                  </div>
                  <ul className="border border-borderGray rounded p-2 h-60 overflow-y-auto">
                    {combination.operations?.map((operation, index) => (
                      <li
                        key={operation.id}
                        className="flex justify-between items-center p-1 px-3 border border-borderGray rounded mb-2"
                      >
              ...
              <span className="flex flex-col items-center my-3">
                ...
                <p className="text-xs text-center text-zinc-500">
                  (Создайте операции для создания комбинации)
                </p>
              </span>
            )}
          </div>
      ...
      <Modal.Footer>
        <div className="flex justify-end">
          <Button
            onClick={onSubmit}
            disabled={operaitions_list?.results?.length <= 0}
          >
            Сохранить
          </Button>
        </div>
